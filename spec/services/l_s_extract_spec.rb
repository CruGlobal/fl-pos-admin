...
describe LSExtract do
  ...
  it('should strip sales data to only the essentials') do
    ...
    sales = lsi.strip_to_named_fields(sales, LightspeedSaleSchema.fields_to_keep)
    expect(sales.first.keys.count).to be == 11
  end

  it('should get all product codes') do
    codes = lsi.get_all_product_codes(example_sales[2]);
    actual = codes.join('|')
  ...
  it('should get all product quantities') do
    codes = lsi.get_all_quantities(example_sales[2]);
    actual = codes.join('|')
    expected = '1|1|1|1'
    expect(actual).to eq(expected)
  end
  ...
  it('should get all unit prices') do
    codes = lsi.get_all_unit_prices(example_sales[2]);
    actual = codes.join('|')
  ...
  it('should get all unit taxes') do
    codes = lsi.get_all_unit_taxes(example_sales[2]);
    actual = codes.join('|')
  ...
  it('should get the taxable order flag') do
    flag = lsi.get_taxable_order_flag(example_sales[2]);
    expected = 'Y'
    expect(flag).to eq(expected)
  end

  it('should get the special order flag') do
    flag = lsi.get_special_order_flag(example_sales[0]);
    expected = 'Y'
    expect(flag).to eq(expected)
    flag = lsi.get_special_order_flag(example_sales[2]);
    expected = 'N'
    expect(flag).to eq(expected)
  end
  ...
  it('should get an address field') do
    ...
    sale = lsi.strip_to_named_fields(sale, LightspeedSaleSchema.fields_to_keep)
    val = lsi.get_address(sale, 'address1')
    expect(val).to be == '33543 Pennbrooke Pkwy'

    val = lsi.get_address(sale, 'foo')
  ...
  it('should get email addresses') do
    sale = example_sales[2]
    sale = lsi.strip_to_named_fields(sale, LightspeedSaleSchema.fields_to_keep)
    val = lsi.get_email_addresses(sale).join('|')
    expect(val).to be == 'bradclay2024@gmail.com'
  end

  it('should get a shipping customers') do
    job = lsi.create_job 16, '2024-12-06', '2024-12-07'
    customers = lsi.get_shipping_customers(job, example_sales)
  ...
  it('should be able to get a shipping address field') do
    ...
    sale = lsi.strip_to_named_fields(sale, LightspeedSaleSchema.fields_to_keep)
    customers = lsi.get_shipping_customers(job, example_sales)
    val = lsi.get_shipping_address(sale, customers, 'address1')
