...
describe LSExtract do
  ...
  xit("should get a report line with shipping") do
    ...
    sale = sales.select { |sale| sale["saleID"] == 250210 }.first
  ...
  xit("should generate an entire report with shipping in places where it needs it") do
    job = lsi.create_job 66, "2025-01-30", "2025-02-05"
    sales = JSON.parse(File.read("#{Rails.root}/spec/fixtures/2025.02.05.grand_rapids.json"))
    report = lsi.generate_report(job, sales)
    report = lsi.process_report(report)
    line1 = report.select { |line| line[:SaleID] == 250210 }.first
    line2 = report.select { |line| line[:SaleID] == 250212 }.first
    expect(line1[:ShipAddressLine1]).to eq("550 Riley St.")
    expect(line2[:ShipAddressLine1]).to be_nil
  end

  xit("should write the sheet properly") do
    job = lsi.create_job 66, "2025-01-30", "2025-02-05"
    sales = JSON.parse(File.read("#{Rails.root}/spec/fixtures/2025.02.05.grand_rapids.json"))
    report = lsi.generate_report(job, sales)
    report = lsi.process_report(report)
    puts report[241].inspect
    context = {}
  ...
  xit('should calculate unit prices properly') do
    job = lsi.create_job 66, "2025-01-30", "2025-02-05"
  ...
  #  xit "should remove item from bundle sale with more quantity" do
  ...
  #  xit "should remove item from bundle sale with more quantity and extra lines" do
  ...
  #  xit "should remove items from bundle refund" do
  #    report = [bundle_sale_line, bundle_refund_line]
  #    processed_report = lsi.process_report(report)
  #    expect(processed_report.count).to eq(0)
  #  end

  #  xit "should remove items from bundle refund with more quantity" do
